  ...
  return (
    <div className="mt-10">
      <Table className="w-full text-white">
        <Thead>
          <Tr className="grid grid-cols-4 bg-richblack-800 py-4 px-6 text-left text-sm font-semibold border-b border-richblack-700">
        ...
        <Tbody>
          {courses.length === 0 ? (
            ...
            courses.map((course) => (
              <Tr
                key={course._id}
                className="grid grid-cols-4 gap-4 border-b border-richblack-700 px-6 py-6 transition-all duration-300 hover:bg-richblack-800 rounded-lg"
              >
                {/* Course Details */}
                <Td className="col-span-2 flex gap-4">
                  <img
                    ...
                    className="h-[100px] w-[150px] rounded-lg object-cover shadow-md border border-richblack-600"
                  />
                    ...
                    <div className="flex gap-3 mt-2">
                      ...
                      {course.status === COURSE_STATUS.DRAFT ? (
                        <span className="flex items-center gap-1 text-xs px-2 py-1 bg-pink-700/20 text-pink-300 rounded-md font-medium">
                          <MdOutlineUnpublished className="text-base" />
                          Drafted
                        </span>
                      ) : (
                ...
                {/* Duration */}
                <Td className="text-sm text-richblack-200 flex items-center">
                  2hr 30min
                </Td>

                {/* Actions */}
                <Td className="flex gap-4 items-center justify-start">
                  {/* Edit Button */}
                  <button
                    disabled={loading}
                    onClick={() =>
